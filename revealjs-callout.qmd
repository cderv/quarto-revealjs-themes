...
This page shows callouts rendered in all Revealjs themes.
...
files <- fs::dir_ls(main_output_path, recurse = TRUE, glob = "*revealjs-callout-*/index.html")
theme <- files |> gsub(pattern = glue::glue("{main_output_path}/revealjs-callout-"), replacement = "", fixed = TRUE) |> fs::path_dir()
files2 <- fs::path(glue::glue("{main_output_path}-2"), glue::glue("revealjs-callout-{theme}"), "index.html")
stopifnot(
  fs::dir_ls(glue::glue("{main_output_path}-2"), recurse = TRUE, glob = "*revealjs-callout-*/index.html") == files2
)
glue::glue("
